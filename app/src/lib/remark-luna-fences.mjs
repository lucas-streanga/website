 ...
 *
 * Rewrites every ```luna block into an <LunaCode code={...} /> element, so an
 * author writes a plain fence and gets the framed, syntax-highlighted card
 * (caption + copy button) instead of a bare <pre>. The raw fence text becomes
 * the `code` prop; an optional title on the info string — ```luna title="foo"
 * — becomes the caption.
 *
...
function toLunaCode(node) {
    const attributes = [
        // Plain-string attribute value → a JS string literal in the compiled
        // component call, so newlines/quotes in the source are escaped for us.
