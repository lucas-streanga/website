...
The reason this works is two-fold: errors are checked, and errors are type-erased. Fully checked errors or exceptions are just too annoying to use in practice. What ends up happening is people abuse sentinel values like `[]` or `''` or `-1` to represent errors. And that sucks, right? Because now we need to read the function documentation, or worse, _body_, to determine what's ACTUALLY an error. And all ergonomics are lost. No message, no stacktrace. Luna's errors keep both - and here's where capabilities sneak back in: actually _reading_ that stacktrace is itself a capability. An error can propagate up the whole call stack, but only the frames you've blessed can crack it open and read the trace. Which is handy, since a stacktrace can leak plenty it shouldn't.
...
Luna is still in its very earliest infancy, and the spec is being refined. The spec and eventual implementation can be viewed at <Link href="https://github.com/lucas-streanga/luna/tree/master">the Luna GitHub Repo</Link>.
