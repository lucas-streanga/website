...
|-|-|-|-|
| Highly Static | Very Good | Usually Bad | C#, C++, Rust, Java |
...
This trait-like system lends us all the garauntees of languages like Rust, without the complexity and mental overhead of inheritence.

# What's More - Obvious Types

I think a lot of languages are scared to add in primitive types as to not bloat the language. But, if we really want stellar ergonomics, we need those primitive types. It can be cumbersome to shoe-horn some functionality behind existing language semantics like inheritence. 

## Capabilities and Secrets

One concept I really wanted to lean into is `capabilities`. Capabilites describe *what* functions can do, but never how. They're simple, data-less tags that we can `use`. We define them like `const revealEnvVars = capability;` Then, we can have some arbtrary function that looks like `const readEnvVars = fn () use (revealEnvVars) {};`.
...
<LunaCode name="handling-errors" />

The reason this works is two-fold: errors are checked, and errors are type-erased. Fully checked errors or exceptions are just too annoying to use in practice. What ends up happening is people abuse sentinel values like `[]` or `''` or `-1` to represent errors. And that sucks, right? Because now we need to read the function documentation, or worse, *body*, to determine what's ACTUALLY an error. And all ergonomics are lost. No message, no stacktrace.

So, we require that a function declares it can throw. And then, we employ - gasp - *inheritence*. Why? Errors naturally fall into a simple tree-like structure, where errors are subtypes of other errors. Maybe `parseError <: runtimeError`, and maybe `intOverflow <: panic`. And, on the topic of panics, we don't need to declare them. Really just about any function can panic with something like `outOfMemory`, so declaring all potential panic points is just wasted noise.
